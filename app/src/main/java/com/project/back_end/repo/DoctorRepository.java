...
//      - Parameters: String name
    @Query("SELECT doc FROM Doctor doc WHERE doc.name LIKE CONCAT('%', :name, '%')")
...
//      - Parameters: String name, String specialty
    ...
    @Query(
        "SELECT doc FROM Doctor doc WHERE LOWER(doc.name) LIKE LOWER(CONCAT('%', :name, '%')) AND LOWER(doc.specialty) = LOWER(:specialty)"
        )
